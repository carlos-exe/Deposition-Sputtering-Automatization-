...
class LX16A:
	...
	@staticmethod
	def initialize(port):
		LX16A.controller = serial.Serial(port=port, baudrate=11200, timeout=.01)
...
try:
    LX16A.initialize("/dev/ttyUSB0")  # Reemplaza con el puerto correcto, como "COM3" en Windows

    # Crear una instancia del servo con el ID que crees que tiene
    servo = LX16A(1)

    # Establecer un ángulo para mover el servo
    servo.move(90)  # Mueve el servo a 90 grados
    print("Moviendo servo a 90 grados")

    # Leer la posición actual para verificar la respuesta
    position = servo.get_position()
    print(f"Posición actual del servo: {position}")

except Exception as e:
    print("Error al comunicarse con el servo:", e)
